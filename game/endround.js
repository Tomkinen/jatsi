...
module.exports = {
  execute: (gameId, selectedResult, games, calculator) => {
    ...
    const checkList = ['ones','twos','threes','fours','fives','sixes','onePair','twoPairs','threeOfAKind','fourOfAKind',
      'smallStraight','largeStraight','fullHouse','chance','yatzy'];
    checkList.forEach((check) => {
      if (selectedResult == check && games[index].gameTable[check + 'Done'] == false) {
    ...
    let calculateSingles = games[index].gameTable.ones+
    ...
    games[index].total = calculateSingles+
