        ...
        methods: {
          ...
          clickNewGame: function (playerCount,numberOfPlayers) {
            if (playerCount<=numberOfPlayers) {
              this.playerNames=[this.player1NameInput,this.player2NameInput,this.player3NameInput,this.player4NameInput];
              this.aiPlayer=[this.player1TypeInput,this.player2TypeInput,this.player3TypeInput,this.player4TypeInput]
              axios.post('api/newgame',{'PLAYERNAME':this.playerNames[playerCount]})
                .then(response=> {
          ...
          newRound: function (gameId,activePlayer) {
            ...
            axios.post('api/newround',{'GAMEID':gameId})
              .then(response=> {
                ...
                if (this.activeAiPlayer=='Y') {
                  ...
                  setTimeout(() => {
                    ...
                    if (this.activeThrowNumber<3) {
                      ...
                      this.aiMessage=this.ui.ai_message_4+' "'+response.data.ai.combination+'"';
                      setTimeout(() => {
                        this.clickEndRound(gameId[activePlayer],response.data.ai.combination);
                      }, 2000);                     
                    }
                  }, 2000);
                }
          ...
          clickEndRound: function (gameId,selectedResult) {
            if (this.dice1!='-' && this.gameTable[this.activePlayer][selectedResult + 'Done'] == false) {
              this.dice1 = '-';
            axios.post('api/endround',{'GAMEID':gameId,'SELECTEDRESULT':selectedResult})
              .then(response=> {
                this[selectedResult+'Value'][this.activePlayer] = response.data.gameTable[selectedResult];
                this.bonusValue[this.activePlayer] = response.data.gameTable.bonus;
                ...
                if (this.activePlayer<this.numberOfPlayers) {
                  if (this.roundNumber[this.activePlayer]==15) {
                    this.endGame(this.gameId[this.activePlayer],this.activePlayer);
                  }
                  this.activePlayer++;
                  this.newRound(this.gameId[this.activePlayer],this.activePlayer);
                }
                else {
                  if (this.roundNumber[this.activePlayer]==15) {
                    this.endGame(this.gameId[this.activePlayer],this.activePlayer);
          ...
          endGame: function (gameId,activePlayer) {
            axios.post('api/endGame',{'GAMEID':gameId})
              .then(response=> {
